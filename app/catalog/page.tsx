...
export default async function CatalogPage(){
  ...
  return <><Header/><main className="wrap page-main public-catalog"><nav className="catalog-breadcrumbs" aria-label="Хлебные крошки"><a href="/">Главная</a><span>/</span><span>Каталог</span></nav><p className="eyebrow">TR HOLOD · ТАШКЕНТ</p><h1>{title}</h1><p className="catalog-lead">{description} В каждой карточке указано назначение оборудования. Цены и остатки демонстрационные; фактические условия требуют подтверждения.</p><SectionLinks/><h2>Все товары · {products.length}</h2><ProductCards products={products}/><script type="application/ld+json" dangerouslySetInnerHTML={{__html:jsonLd({'@context':'https://schema.org','@type':'CollectionPage',name:title,url:absoluteUrl('/catalog'),mainEntity:{'@type':'ItemList',itemListElement:products.map((p,i)=>({'@type':'ListItem',position:i+1,name:p.title,url:absoluteUrl(productPath(p))}))}})}}/></main><Footer/></>;
}
