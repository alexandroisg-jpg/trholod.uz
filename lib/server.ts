import { updateRefrigerantStudio } from './refrigerant-studio-update';
...
export async function seed(){
 const d=db();
 if(await d.prepare('SELECT value FROM settings WHERE key=?').bind('seed_originals_v4_release').first()){await updateRefrigerantPackaging(d);await updateRefrigerantStudio(d);return;}
 ...
 await updateRefrigerantStudio(d);
}
